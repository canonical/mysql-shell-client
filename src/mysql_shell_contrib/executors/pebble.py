...
class PebbleExecutor(BaseExecutor):
    """Pebble executor for the MySQL Shell."""
    ...
    @staticmethod
    def _strip_password(exc: ops.pebble.Error):
        """Strip passwords from SQL scripts."""
        if not isinstance(exc, ops.pebble.ExecError):
            return exc

        password_pattern = re.compile("(?<=IDENTIFIED BY ')[^']+(?=')")
        password_replace = "*****"  # noqa: S105

        for index, value in enumerate(exc.command):
            if "IDENTIFIED BY" in value:
                exc.command[index] = re.sub(password_pattern, password_replace, value)

        return exc

    def set_container(self, container: ops.model.Container) -> None:
